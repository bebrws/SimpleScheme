...
static Scheme_Object *pixel(int argc, Scheme_Object *argv[]);
...
void scheme_init_error(Scheme_Startup_Env *env)
{
  ...
  ADD_PRIM_W_ARITY("pixel",    pixel,   6, 6, env);
...
static Scheme_Object *
pixel(int argc, Scheme_Object *argv[])
{
  int x = SCHEME_INT_VAL(argv[0]);
  int y = SCHEME_INT_VAL(argv[1]);
  int r = SCHEME_INT_VAL(argv[2]);
  int g = SCHEME_INT_VAL(argv[3]);
  int b = SCHEME_INT_VAL(argv[4]);
  int a = SCHEME_INT_VAL(argv[5]);
  
  pixels[((x+(y*300))*4) + 0] = r;
  pixels[((x+(y*300))*4) + 1] = g;
  pixels[((x+(y*300))*4) + 2] = b;
  pixels[((x+(y*300))*4) + 3] = a;
